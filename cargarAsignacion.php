...
echo 'carrera_seleccionada: '.$carrera_seleccionada.'<br>';
echo 'nivel_carrera_seleccionada: '.$nivel_carrera_seleccionada.'<br>';
echo 'grupo_seleccionado: '.$grupo_seleccionado.'<br>';
...
$contM=0;
$distinto=0;
if ($grupo_seleccionado != 0 && $nivel_carrera_seleccionada == '' && $carrera_seleccionada == '' || $grupo_seleccionado != 0 && $nivel_carrera_seleccionada == 0 && $carrera_seleccionada == 0){
$contA=0;
$contM=0;
//echo 'entro if 1'.'<br>';
	$val = pg_query("SELECT id_alumno FROM alumno INNER JOIN alumnos_por_grupo ON(alumnos_por_grupo.alumno_fk = alumno.id_alumno) WHERE grupo_fk = '$grupo_seleccionado' ORDER BY id_alumno");
	while($row=pg_fetch_array($val,NULL,PGSQL_ASSOC)){
		$graduado_asignado[$a] = $row['id_alumno'];
		$contA++;
		$a++;
	}
	$val1 = pg_query("SELECT id_alumno,nombre_alumno,apellido_alumno,nombre_carrera FROM alumno INNER JOIN carrera ON(carrera.id_carrera = alumno.carrera_alumno) ORDER BY id_alumno");
...
// echo 'contA 1 if: '.$contA.'<br>';
// echo 'contM 1 if: '.$contM.'<br>';
}
if ($grupo_seleccionado != 0 && $nivel_carrera_seleccionada != 0 && $carrera_seleccionada == 0){
$contA=0;
$contM=0;
//echo 'entro if 2'.'<br>';
	$val2 = pg_query("SELECT id_alumno FROM alumno INNER JOIN carrera ON(carrera.id_carrera = alumno.carrera_alumno) INNER JOIN alumnos_por_grupo ON(alumnos_por_grupo.alumno_fk = alumno.id_alumno) WHERE grupo_fk = '$grupo_seleccionado' AND nivel_carrera_fk = '$nivel_carrera_seleccionada' ORDER BY id_alumno");
	while($row2=pg_fetch_array($val2,NULL,PGSQL_ASSOC)){
		$graduado_asignado[$a] = $row2['id_alumno'];
		$contA++;
		$a++;
	}
	$val3 = pg_query("SELECT id_alumno,nombre_alumno,apellido_alumno,nombre_carrera FROM alumno INNER JOIN carrera ON(carrera.id_carrera = alumno.carrera_alumno) WHERE nivel_carrera_fk = '$nivel_carrera_seleccionada' ORDER BY id_alumno");
	while($row3=pg_fetch_array($val3,NULL,PGSQL_ASSOC)){
		$graduados_mostrar[$na] = $row3['id_alumno'];
		$nombre_alumno[$na] = $row3['nombre_alumno'];
		$apellido_alumno[$na] = $row3['apellido_alumno'];
		$nombre_carrera[$na] = $row3['nombre_carrera'];
		$contM++;
		$na++;
	}
// echo 'contA if 2: '.$contA.'<br>';
// echo 'contM if 2: '.$contM.'<br>';
}
if ($grupo_seleccionado != 0 && $nivel_carrera_seleccionada != 0 && $carrera_seleccionada != 0 || $grupo_seleccionado != 0 && $nivel_carrera_seleccionada == 0 && $carrera_seleccionada != 0){
$contA=0;
$contM=0;
// echo 'entro if 3'.'<br>';
	//$val4 = pg_query("SELECT id_alumno,nombre_alumno,apellido_alumno,nombre_carrera FROM alumno INNER JOIN carrera ON(carrera.id_carrera = alumno.carrera_alumno) INNER JOIN alumnos_por_grupo ON(alumnos_por_grupo.alumno_fk = alumno.id_alumno) WHERE grupo_fk = '$grupo_seleccionado' AND nivel_carrera_fk = '$nivel_carrera_seleccionada' AND carrera_alumno = '$carrera_seleccionada' ORDER BY id_alumno");
$val4 = pg_query("SELECT id_alumno FROM alumno INNER JOIN carrera ON(carrera.id_carrera = alumno.carrera_alumno) INNER JOIN alumnos_por_grupo ON(alumnos_por_grupo.alumno_fk = alumno.id_alumno) WHERE grupo_fk = '$grupo_seleccionado' AND carrera_alumno = '$carrera_seleccionada' ORDER BY id_alumno");
	while($row4=pg_fetch_array($val4,NULL,PGSQL_ASSOC)){
		$graduado_asignado[$a] = $row4['id_alumno'];
		$contA++;
		$a++;
	}
	$val5 = pg_query("SELECT id_alumno,nombre_alumno,apellido_alumno,nombre_carrera FROM alumno INNER JOIN carrera ON(carrera.id_carrera = alumno.carrera_alumno) WHERE carrera_alumno = '$carrera_seleccionada' ORDER BY id_alumno");
	while($row5=pg_fetch_array($val5,NULL,PGSQL_ASSOC)){
		$graduados_mostrar[$na] = $row5['id_alumno'];
		$nombre_alumno[$na] = $row5['nombre_alumno'];
		$apellido_alumno[$na] = $row5['apellido_alumno'];
		$nombre_carrera[$na] = $row5['nombre_carrera'];
		$contM++;
		$na++;
	}
// echo 'contA if 3: '.$contA.'<br>';
// echo 'contM if 3: '.$contM.'<br>';
}
//Tabla que muestra la cantidad de asignados y la cantidad de los que faltan asignar.
