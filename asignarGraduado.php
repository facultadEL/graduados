...
if ($grupo_fk != 0 && $id_nivel_carrera_seleccionada == '' && $id_carrera_seleccionada == '' || $grupo_fk != 0 && $id_nivel_carrera_seleccionada == 0 && $id_carrera_seleccionada == 0){
echo 'entro if 1'.'<br>';
	$cantidadAlumnos = pg_query("SELECT count(id_alumno) as cant FROM alumno INNER JOIN carrera ON(carrera.id_carrera = alumno.carrera_alumno) INNER JOIN alumnos_por_grupo ON(alumnos_por_grupo.alumno_fk = alumno.id_alumno)");
	$rowCant=pg_fetch_array($cantidadAlumnos,NULL,PGSQL_ASSOC);
		$cant = $rowCant['cant'];
}
if ($grupo_fk != 0 && $id_nivel_carrera_seleccionada != 0 && $id_carrera_seleccionada == 0){
echo 'entro if 2'.'<br>';
	$cantidadAlumnos = pg_query("SELECT count(id_alumno) as cant FROM alumno INNER JOIN carrera ON(carrera.id_carrera = alumno.carrera_alumno) WHERE nivel_carrera_fk = '$id_nivel_carrera_seleccionada'");
	$rowCant=pg_fetch_array($cantidadAlumnos,NULL,PGSQL_ASSOC);
		$cant = $rowCant['cant'];
}
if ($grupo_fk != 0 && $id_nivel_carrera_seleccionada != 0 && $id_carrera_seleccionada != 0){
